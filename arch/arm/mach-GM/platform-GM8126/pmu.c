...
/* SPI
*/
static pmuReg_t regSPIArray[] = {
 /* CLK init */
 /* reg_off  bit_masks  lock_bits     init_val    init_mask */
    {0x78,  (0x3F << 0), (0x3F << 0), (0x02 << 0), (0x3F << 0)},
...
static pmuRegInfo_t spi_clk_pinmux_info = {
    "spi_clk_pinmux", 
    ARRAY_SIZE(regSPIArray),
...
{    
    ...
    for (i = 0; i < ARRAY_SIZE(regSPIArray) ; ++i) {
        if (spi_clk_pinmux_info.pRegArray[i].reg_off == 0x54) {
#ifdef CONFIG_SPI_FTSSP010_CS0
            spi_clk_pinmux_info.pRegArray[i].bits_mask |= (0x3 << 4);
            spi_clk_pinmux_info.pRegArray[i].lock_bits |= (0x3 << 4); 
            spi_clk_pinmux_info.pRegArray[i].init_val  |= (0x0 << 4);
            spi_clk_pinmux_info.pRegArray[i].init_mask |= (0x3 << 4);
#endif
#ifdef CONFIG_SPI_FTSSP010_CS1
            spi_clk_pinmux_info.pRegArray[i].bits_mask |= (0x3 << 6);
            spi_clk_pinmux_info.pRegArray[i].lock_bits |= (0x3 << 6); 
            spi_clk_pinmux_info.pRegArray[i].init_val  |= (0x0 << 6);
            spi_clk_pinmux_info.pRegArray[i].init_mask |= (0x3 << 6);
#endif
        }

        if (spi_clk_pinmux_info.pRegArray[i].reg_off == 0x60) {
#ifdef CONFIG_SPI_FTSSP010_CS2
            spi_clk_pinmux_info.pRegArray[i].bits_mask |= (0x7 << 6);
            spi_clk_pinmux_info.pRegArray[i].lock_bits |= (0x7 << 6); 
            spi_clk_pinmux_info.pRegArray[i].init_val  |= (0x0 << 0);
            spi_clk_pinmux_info.pRegArray[i].init_mask |= (0x7 << 0);
#endif
#ifdef CONFIG_SPI_FTSSP010_CS3
            spi_clk_pinmux_info.pRegArray[i].bits_mask |= (0x7 << 3);
            spi_clk_pinmux_info.pRegArray[i].lock_bits |= (0x7 << 3); 
            spi_clk_pinmux_info.pRegArray[i].init_val  |= (0x0 << 3);
            spi_clk_pinmux_info.pRegArray[i].init_mask |= (0x7 << 3);
#endif
#ifdef CONFIG_SPI_FTSSP010_CS4
            spi_clk_pinmux_info.pRegArray[i].bits_mask |= (0x7 << 6);
            spi_clk_pinmux_info.pRegArray[i].lock_bits |= (0x7 << 6); 
            spi_clk_pinmux_info.pRegArray[i].init_val  |= (0x0 << 6);
            spi_clk_pinmux_info.pRegArray[i].init_mask |= (0x7 << 6);
#endif
#ifdef CONFIG_SPI_FTSSP010_CS5
            spi_clk_pinmux_info.pRegArray[i].bits_mask |= (0x7 << 9);
            spi_clk_pinmux_info.pRegArray[i].lock_bits |= (0x7 << 9); 
            spi_clk_pinmux_info.pRegArray[i].init_val  |= (0x0 << 9);
            spi_clk_pinmux_info.pRegArray[i].init_mask |= (0x7 << 9);
#endif
        }
    }
    spi_fd = ftpmu010_register_reg(&spi_clk_pinmux_info);
    if (unlikely(spi_fd < 0)){
        printk("SPI registers to PMU fail! \n");
    } 
