...
extern pid_t   PM0_Pid;
extern unsigned long  IdleTime;
struct device    *SataPortDev1;
int     SataRWState1 = SATA_STATE_ACTIVE;
    ...
    switch( SataRWState1 )
    {
        case SATA_STATE_INACTIVE:
            ...
            if( MinuteCount >= IdleTime )
            {
                ...
                printk("\n---------- SATA-1 STANDBY -----------\n");
                if( PM0_Pid )
                    kill_proc( PM0_Pid, SIGUSR1, 1 );
                SataRWState1 = SATA_STATE_STANDBY;
            }
            break;
        case SATA_STATE_ACTIVE:
            SataRWState1 = SATA_STATE_INACTIVE;
            MinuteCount = 0;
            break;
...
EXPORT_SYMBOL_GPL(SataPortDev1);
EXPORT_SYMBOL_GPL(SataRWState1);
//-
...
void lepus_sata_setup_port1(struct ata_ioports *port, unsigned long base)
{
...
//	writel(0x03,IO_ADDRESS(SL2312_SATA_BASE)+0x18);					// Enable SATA salve mode	
	...
	lepus_sata_setup_port1(&probe_ent->port[0], probe_ent->mmio_base );
