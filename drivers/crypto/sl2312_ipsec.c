...
#define     IPSEC_MAX_PACKET_LEN    2048 + 256
...
static void ipsec_hw_auth(unsigned char *ctrl_pkt,int ctrl_len,unsigned char *data_pkt, int data_len, unsigned int tqflag,
                            unsigned char *out_pkt,int *out_len);
static void ipsec_hw_cipher(unsigned char *ctrl_pkt,int ctrl_len,unsigned char *data_pkt, int data_len, unsigned int tqflag,
                            unsigned char *out_pkt,int *out_len);
static void ipsec_hw_fcs(unsigned char *ctrl_pkt,int ctrl_len,unsigned char *data_pkt, int data_len, unsigned int tqflag,
                            unsigned char *out_pkt,int *out_len);
...
void ipsec_hw_auth(unsigned char *ctrl_pkt,int ctrl_len,unsigned char *data_pkt, int data_len, unsigned int tqflag,
                            unsigned char *out_pkt,int *out_len)
{
//    unsigned int        ipsec_status;
//    unsigned int        i;   
...
void ipsec_hw_fcs(unsigned char *ctrl_pkt,int ctrl_len,unsigned char *data_pkt, int data_len, unsigned int tqflag,
                            unsigned char *out_pkt,int *out_len)
{
...
//	ipsec_write_reg(IPSEC_TXDMA_BUF_ADDR,(unsigned int)pkt_ptr,0xffffffff);    
	...
	for (i=0; i<desc_cnt ;i++)
	{
		...
		tx_desc->frame_ctrl.bits.buffer_size = TX_BUF_SIZE; /* descriptor byte count */
        tx_desc->flag_status.bits_tx_flag.tqflag = tqflag;    /* set tqflag */

#if (ZERO_COPY == 0)
		memcpy((char *)(tx_desc->buf_adr+tx_buf_virtual_base),pkt_ptr,TX_BUF_SIZE); /* copy packet to descriptor buffer address */
        pkt_ptr = &pkt_ptr[TX_BUF_SIZE];
#else
        tx_desc->buf_adr = (unsigned int)pkt_ptr;
        pkt_ptr = (unsigned char *)((unsigned int)pkt_ptr + TX_BUF_SIZE);
#endif
        
        if ( (desc_cnt==1 )&& (last_desc_byte_cnt==0) )
        {
            sof = 0x03; /*only one descriptor*/
        }    
		else if ( (last_desc_byte_cnt==0) && (i==(desc_cnt-1)) )
		{
    ...
    if (last_desc_byte_cnt > 0) /* the last descriptor */
    {
		tx_desc->frame_ctrl.bits.buffer_size = last_desc_byte_cnt; /* descriptor byte count */

        tx_desc->flag_status.bits_tx_flag.tqflag = tqflag;    /* set tqflag */

#if (ZERO_COPY == 0)
		memcpy((char *)(tx_desc->buf_adr+tx_buf_virtual_base),pkt_ptr,last_desc_byte_cnt); /* cop->y packet to descriptor buffer address */
#else
        tx_desc->buf_adr = (unsigned int)pkt_ptr;
#endif
		tx_desc->next_desc.bits.eofie = 1;
		if (desc_cnt > 0)
		    tx_desc->next_desc.bits.sof_eof = 0x01; /*the last descriptor*/
		else
		    tx_desc->next_desc.bits.sof_eof = 0x03; /*only one descriptor*/
