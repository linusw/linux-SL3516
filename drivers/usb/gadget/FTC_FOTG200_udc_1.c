...
FTC_ep_enable(struct usb_ep *_ep, const struct usb_endpoint_descriptor *desc)
{
	...
	DBG_CTRLL("+FTC_ep_enable() : _ep = %x desc = %x ep->desc= %x\n",(u32) _ep, (u32) desc, (u32) ep->desc);
        DBG_CTRLL("ep->num %x desc->bEndpointAddress %x \n",ep->num,desc->bEndpointAddress); 
  //<1>.Checking input
	if (!_ep || !desc || ep->desc
...
//                retval = dma_alloc_coherent(&the_controller_1->gadget.dev,
//                                bytes, dma, gfp_flags);
                  retval = pci_alloc_consistent(NULL,bytes, dma);
                  
                printk("###retval %x bytes %x dma %x gfp_flags %x\n",retval,bytes,dma,gfp_flags);
	//} 
	//else
	//	retval = kmalloc(bytes, gfp_flags);
...
//		consistent_free(buf, bytes, dma);//Bruce;;05032005;;
                dma_free_coherent(&the_controller_1->gadget.dev, bytes, buf, dma);
...
//            }	
		wDMABuffer = dma_map_single((void *)dev, pu8Buffer,  temp*16, //USB_EPX_BUFSIZ 
			bdir ? PCI_DMA_TODEVICE : PCI_DMA_FROMDEVICE);
                ...
                DBG_CTRLL("CX_dma_Directly wDMABuffer %x\n",wDMABuffer);
...
//		DBG(dev, "request interrupt failed\n");
	...
	the_controller_1->gadget.dev.dma_mask = (void *)0xffffffff;
	the_controller_1->gadget.dev.coherent_dma_mask = 0xffffffff;
	device_register (&(the_controller_1->gadget.dev));
#endif
