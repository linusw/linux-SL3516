...
#define GMAX_TX_INTR_DISABLED			1 /* suggest by Andge 20080715*/
#define ENABLE_TSO						1
...
//		writel(0x5787a5f0,GMAC_GLOBAL_BASE_ADDR+0x1c);//For 3512 Switch Board 
        writel(0xa7f0c7a0,GMAC_GLOBAL_BASE_ADDR+0x1c);
		writel(0x55557777,GMAC_GLOBAL_BASE_ADDR+0x20);//For 3512 Switch Board 
	}
	else if((chip_id == 0x3512C3)) // +20081011 nelson for A3 Version CPU
	{
		writel(0xa7f0a7a0,GMAC_GLOBAL_BASE_ADDR+0x1c); //benson suggest
        writel(0x55557777,GMAC_GLOBAL_BASE_ADDR+0x20);
	}
	else // +20081011 nelson for other version. The setting should be work
	{
		writel(0xa7f0a7a0,GMAC_GLOBAL_BASE_ADDR+0x1c); //benson suggest
        writel(0x55557777,GMAC_GLOBAL_BASE_ADDR+0x20);
	}
...
#endif
				toe->gmac[i].phy_vendor = gmac_get_phy_vendor(toe->gmac[i].phy_addr);
				if (toe->gmac[i].phy_vendor != 0 && toe->gmac[i].phy_vendor != 0xffffffff)
                                   {
                                        printk(" ---> phy_vendor = 0x%X\n", toe->gmac[i].phy_vendor);
					toe->gmac[i].existed = GMAC_EXISTED_FLAG;
                                   }
			}
		}
...
#if defined(CONFIG_SL351x_NAT) || defined(CONFIG_SL351x_RXTOE) || defined(CONFIG_SL351X_BR)
		sl351x_hash_init();
#else
	{
...
//	spin_lock(&tp->tx_mutex);	
	...
	{
//		spin_unlock(&tp->tx_mutex);
    		printk("\nSW TxQ Read pointer does not move %.8x ... \n", rwptr.bits32); 
		printk("Free_DESC: %d, snd_pages: %d\n", free_desc, snd_pages);	
		printk("SW TQ finished: %d, rptr: %d, wptr: %d\n", swtxq->finished_idx, rptr, wptr);	
		printk("SW TQ first desc: %.8x, current desc: %.8x\n", 
			readl(TOE_GMAC0_DMA_BASE+GMAC_DMA_TX_FIRST_DESC_REG), 
			readl(TOE_GMAC0_DMA_BASE+GMAC_DMA_TX_CURR_DESC_REG));
		isPtr->tx_dropped++;
...
//		spin_unlock(&tp->tx_mutex);
		...
		printk("SW TQ first desc: %.8x, current desc: %.8x\n", 
			readl(TOE_GMAC0_DMA_BASE+GMAC_DMA_TX_FIRST_DESC_REG), 
			readl(TOE_GMAC0_DMA_BASE+GMAC_DMA_TX_CURR_DESC_REG));
		printk("TXQ regs: %.8x, %.8x,\n%.8x, %.8x, %.8x, %.8x\n", 
  				readl(TOE_GMAC0_DMA_BASE+GMAC_DMA_TX_FIRST_DESC_REG), 
  				readl(TOE_GMAC0_DMA_BASE+GMAC_DMA_TX_CURR_DESC_REG), 
  				readl(TOE_GMAC0_DMA_BASE+GMAC_DMA_TX_DESC_WORD0_REG), 
  				readl(TOE_GMAC0_DMA_BASE+GMAC_DMA_TX_DESC_WORD1_REG),
  				readl(TOE_GMAC0_DMA_BASE+GMAC_DMA_TX_DESC_WORD2_REG),
  				readl(TOE_GMAC0_DMA_BASE+GMAC_DMA_TX_DESC_WORD3_REG));
		isPtr->tx_dropped++;
		printk("GMAC %d No available descriptor!\n", tp->port_id);
		netif_stop_queue(dev);
		return 1;
...
//		consistent_sync((void *)curr_desc, sizeof(GMAC_TXDESC_T), PCI_DMA_FROMDEVICE);
		...
		/* read back descriptors again, jeanson */
		mb();
		word0 = curr_desc->word0.bits32;
		word1 = curr_desc->word1.bits32;
		word2 = curr_desc->word2.bits32; 
		word3 = curr_desc->word3.bits32; 
		mb(); 		
		...
		if (tp->port_id == 0)
		{
			if ((status4 & G0_INTR4_BITS) && (tp->intr4_enabled & G0_INTR4_BITS))
			{
				...
				if (status4 & GMAC0_RX_PAUSE_ON_INT_BIT)
					{
						tp->rx_pause_on_cnt++;
						printk("GMAC0 Rx pause frame on\n");
					}
				if (status4 & GMAC0_TX_PAUSE_ON_INT_BIT)
					{
						tp->tx_pause_on_cnt++;
						printk("GMAC0 Tx pause frame on \n");
					}
				if (status4 & GMAC0_RX_PAUSE_OFF_INT_BIT)
					{
						tp->rx_pause_off_cnt++;
						printk("GMAC0 Rx pause frame off\n");
					}
				if (status4 & GMAC0_TX_PAUSE_OFF_INT_BIT)
					{
						tp->rx_pause_off_cnt++;
						printk("GMAC0 Tx pause frame off\n");
					}
		...
		if (tp->port_id == 0)
		{
			if ((status4 & G0_INTR4_BITS) && (tp->intr4_enabled & G0_INTR4_BITS))
			{
				...
				if (status4 & GMAC0_RX_PAUSE_ON_INT_BIT)
					{
						tp->rx_pause_on_cnt++;
						printk("Gmac0 Rx pause on\n");
					}
				if (status4 & GMAC0_TX_PAUSE_ON_INT_BIT)
					{
						tp->tx_pause_on_cnt++;
						printk("Gmac0 Tx pause on\n");
					}
				if (status4 & GMAC0_RX_PAUSE_OFF_INT_BIT)
					{
						tp->rx_pause_off_cnt++;
						printk("Gmac0 Rx pause off\n");
					}
				if (status4 & GMAC0_TX_PAUSE_OFF_INT_BIT)
					{
						tp->rx_pause_off_cnt++;
						printk("Gmac0 Tx pause off\n");
					}
				if (status4 & GMAC0_RX_OVERRUN_INT_BIT)
					tp->rx_overrun_cnt++;
				if (status4 & GMAC0_STATUS_CHANGE_INT_BIT)
...
void gmac_set_phy_status(struct net_device *dev)
{
	...
	unsigned int    reg_val, ability,wan_port_id, linked_1G=0;
	unsigned int    i = 0;
...
//		SPI_write(2,0,0x10,rcv_mask);			// Enable Receive
  ...
  /* store the phy vendor info so following functions could use this
  as identification, jeanson */
	//tp->phy_vendor = gmac_get_phy_vendor(tp->phy_addr);
	printk("GMAC-%d Addr %d Vendor ID: 0x%08x\n", tp->port_id, tp->phy_addr, tp->phy_vendor);

/* do not depened on the phy_mode setting, depend on phy_vendor instead,
   jeanson */
   
  switch (tp->phy_vendor)
  {
  	case MARVELL_88E1111:	/* assuming Linked @ 1000Mbps Full */
  		printk("Marvell 88E1111 detected\n");
  		mii_write(tp->phy_addr,0x04,0x05e1); /* advertisement 100M full duplex, pause capable on */
			...
			tp->phy_mode = GMAC_PHY_RGMII_1000;
			break;
		
		case ICPLUS_AC101A:	/* assuming 100Mbps full */
			printk("IC+ AC101A detected\n"); 
			mii_write(tp->phy_addr,0x04,0x05e1); /* advertisement 100M full duplex, pause capable on */
			tp->phy_mode = GMAC_PHY_MII; 
			break;
		
		default:	/* assuming 1000Mbps full */
			mii_write(tp->phy_addr,0x04,0x05e1); /* advertisement 100M full duplex, pause capable on */
			mii_write(tp->phy_addr,0x09,0x0300); /* advertise 1000M full/half duplex */
			break;
	}

	mii_write(tp->phy_addr, 0x18, 0x004441);	/* phy active led Andge */
	...
	/* read back the setting of the (G)MII/R(G)MII interface, jeanson */
	printk("MII Link status: %.8x\t", mii_read(tp->phy_addr, 0x01)); 
	/* print out the PHY link status */
	printk("Phy status: %.8x\n", ability); 
	/* end read back */
	
	/* check the peer link status */
	reg_val = mii_read(tp->phy_addr,10);
	printk("MII REG 10 = 0x%x\n",reg_val);
	
	if (tp->phy_mode != GMAC_PHY_MII)
		{
...
void gmac_get_phy_status(struct net_device *dev)
{
	...
	unsigned int    reg_val,ability,wan_port_id, linked_1GB=0;
	unsigned int  	check_speed;//micle add 20081201 --10M not work well
...
phy_status:
	...
	if ((reg_val & 0x0024) == 0x0024) /* link is established and auto_negotiate process completed */
	{
		...
		//micle add 20081201 --10M not work well
		check_speed = (mii_read(tp->phy_addr,0x11) )>> 13;
		if (tp->phy_mode != GMAC_PHY_MII)
			{ /* read back link partner's status if with 1000Mbps GMII/RGMII mode */
				reg_val = mii_read(tp->phy_addr,10);
				if ((reg_val & 0x0800) == 0x0800)
				{
					status.bits.duplex = 1;
					status.bits.speed = 2;
					linked_1GB = 1;	/* linked @ 1Gbps */
					if (status.bits.mii_rmii == GMAC_PHY_RGMII_100)
						status.bits.mii_rmii = GMAC_PHY_RGMII_1000;
				}else if ((reg_val & 0x0400) == 0x0400)
				{
		...
		if (!linked_1GB)
		{
			...
			#endif
			/*//micle mask 20081201
			if ((ability & 0x08)==0x08) 
			{
				status.bits.duplex = 1;
				status.bits.speed = 1;
			}else if ((ability & 0x04)==0x04) 
			{
				status.bits.duplex = 0;
				status.bits.speed = 1;
			}else if ((ability & 0x02)==0x02) 
			{
				status.bits.duplex = 1;
				status.bits.speed = 0;
			}else if ((ability & 0x01)==0x01) 
			{
				status.bits.duplex = 0;
				status.bits.speed = 0;
			}
			*/
			...
			else if (check_speed ==0x2) 
			{
				status.bits.duplex = 0;
				status.bits.speed = 1;
			}
			else if (check_speed ==0x1) 
			{
				status.bits.duplex = 1;
				status.bits.speed = 0;
			}
			else if (check_speed ==0) 
			{
				status.bits.duplex = 0;
				status.bits.speed = 0;
			}
...
static int gmac_netdev_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
    ...
    struct mii_ioctl_data *data = if_mii(rq); //vanghan
    u32 mii_regval;
	...
	case SIOCGMIIPHY:	/* Get the address of the PHY in use. */
	  data->phy_id=0x01;//phy address
        break;

	case SIOCGMIIREG:	/* Read the specified MII register. */
		spin_lock_irq(&tp->lock);  //micle ++ 20081127 MII_REG
		mii_regval=mii_read(tp->phy_addr,data->reg_num);//micle ++ 20081127
		spin_unlock_irq(&tp->lock);//micle ++ 20081127
		data->val_out = mii_regval;	//micle ++ 20081127
		break;

	case SIOCSMIIREG:	/* Write the specified MII register */
		spin_lock_irq(&tp->lock);//micle ++ 20081127
		mii_write(tp->phy_addr,data->reg_num, data->val_in);//micle ++ 20081127
		spin_unlock_irq(&tp->lock);	//micle ++ 20081127
		break;
...
static void gmac_reset_task(struct net_device *dev)	
{
	...
	// debug, jeanson
	printk("Gmac in doing workaround... \n"); 
