...
#else
	i2c_writereg(0x22,0x03,left > 94 ? 94 : left);
	//printk("\nvol L(%x)",i2c_readreg(0x23, 0x03));
	i2c_writereg(0x22,0x04,right > 94 ? 94 : right);
...
void cs4341_init(void)
{
	...
	for (tmp = 0; tmp < 200; tmp++) {
			...
			i2c_writereg(0x22,0x03,0x00);
			i2c_writereg(0x22,0x04,0x00);
...
{
	int                     tmp;
	unsigned int opcode=0, old_opcode=0, w_start=0, i;
	...
	tmp=READ_DMA_REG(DMA_INT_TC); 
			...
			{				
				...
				//ssp_i2s.wt_curr = (ssp_i2s.tx_curr/SBUF_SIZE);
				w_start = (ssp_i2s.tx_curr/SBUF_SIZE);
				...
				else
				{
...
#if (DEF_16_BIT==1)		  							
					ttt = 1;
					//for(i=0;i<old_opcode;i++)
					//{
					//	printk("* %d *",((w_start+i)%LLP_SIZE));
					//		memset(__va(ssp_i2s.LLPT[((w_start+i)%LLP_SIZE)].src_addr), 0, SBUF_SIZE/8);	
					//		ssp_i2s.LLPT[((w_start+i)%LLP_SIZE)].ctrl_size = ((SBUF_SIZE/4)|(0x040a<<16));
					//}
#else
					...
					//{
					//	memset(__va(ssp_i2s.LLPT[((w_start+i)%LLP_SIZE)].src_addr), 0, SBUF_SIZE/8);	
					//	ssp_i2s.LLPT[((w_start+i)%LLP_SIZE)].ctrl_size = (((SBUF_SIZE/4)/2)|(0x040a<<16));
					//}
#endif	
	...
	//		if(ssp_i2s.in_cont<LLP_SIZE)
			{
				...
				{		
					...
					if((LLP_SIZE - ssp_i2s.in_cont)>0)
					{
						...
						if((ssp_i2s.wt_curr > ((3*LLP_SIZE)/4))&&(ssp_i2s.in_bufok == 0))
						{
								ssp_i2s.in_bufok = 1;
