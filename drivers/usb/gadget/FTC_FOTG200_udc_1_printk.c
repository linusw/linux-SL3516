...
FTC_ep_enable(struct usb_ep *_ep, const struct usb_endpoint_descriptor *desc)
{
	...
	printk("+FTC_ep_enable() : _ep = %x desc = %x ep->desc= %x\n",(u32) _ep, (u32) desc, (u32) ep->desc);
        printk("ep->num %x desc->bEndpointAddress %x \n",ep->num,desc->bEndpointAddress); 
  //<1>.Checking input
	if (!_ep || !desc || ep->desc
...
static int CX_dma_Directly(u8 *pu8Buffer, u32 u8Num,u8 bdir)
{
    ...
    u8             u8fifo_n;
	struct FTC_udc	*dev = the_controller_1;    
...
//            }	
		wDMABuffer = dma_map_single((void *)dev, pu8Buffer,  temp*16, //USB_EPX_BUFSIZ 
			bdir ? PCI_DMA_TODEVICE : PCI_DMA_FROMDEVICE);

                printk("CX_dma_Directly wDMABuffer %x\n",wDMABuffer);
...
//	                         mUsbEPinZeroSet(ep->num); //Faraday define !!!
	        ...
	        if ((ep->num)==0)	
	           ...
	           {//For EP1~EP4
	            ...
	            printk(">>>start_dma ==>(mUsbFIFOOutByteCount=0x%x(ep->num=0x%x)(MaxPackSize=0x%x))\n"
	    ...
	    mUsbDMA2FIFOSel(FIFO_Sel);
	    mUsbDmaAddr(start);
	    printk(">>>(mUsbDmaConfigRd=0x%x(Request Length=0x%x))\n",mUsbDmaConfigRd(),req->req.length);
	    printk(">>>(mUsbDMA2FIFOSel=0x%x)\n",mUsbDMA2FIFORd());
	    printk(">>>(mUsbDmaAddr=0x%x)\n",mUsbDmaAddrRd());
 ...
 //<1>.Check request & ep & dev
  ...
  //Check CX 0 bytes
    if (req->req.length==0)
       {if (ep->num==0)
       	   {//request Control Transfer 0 byte
       	   	 ...
       	   	 printk(">>> FTC_queue => return (set configuration)\n");
       	   	 return 0;
       	   	}
	...
	if (ep->dma && _req->dma == DMA_ADDR_INVALID) {
		DBG_CTRLL("....dma_map_single len = %d\n",_req->length);
...
//            }	
		_req->dma = dma_map_single((void *)dev, _req->buf, temp*16, //USB_EPX_BUFSIZ,  
			ep->is_in ? PCI_DMA_TODEVICE : PCI_DMA_FROMDEVICE);
 ...
 //<3>.For control-in => Fource short packet
	...
	if (list_empty(&ep->queue) && likely(!ep->stopped)  ) {
        ...
        printk(">>> dev->EPUseDMA = 0x%x (ep->num=%x)\n",dev->EPUseDMA,ep->num);		
    ...
    if (likely((ep->num)>0))//Open the FIFO interrupt
       {
        ...
        printk(">>> Enable EP-%x Interrupt (Register=0x%x)(Length=0x%x)...\n"
...
static void vUsb_ep0setup(struct FTC_udc *dev)
{
	...
	printk(" u32UsbCmd[0] %x u32UsbCmd[1] %x \n",u32UsbCmd[0],u32UsbCmd[1]);
	mUsbDMA2FIFOSel(FOTG200_DMA2FIFO_Non);
	memcpy(u8UsbCmd,u32UsbCmd,8);

   	printk("L%x, EP0Cmd:%02x %02x %02x %02x %02x %02x %02x %02x\n", dev->u8LineCount ++,
...
static void vUsbHandler(struct FTC_udc	*dev)//FOTG200.ok
{
	...
	printk("usb_interrupt_level1:0x%x\n",dev->usb_interrupt_level1);
 ...
 //----- Group Byte 2 ---------
	...
	printk("usb_interrupt_Origan:0x%x\n",usb_interrupt_Origan);		
	printk("usb_interrupt_Mask:0x%x\n",usb_interrupt_Mask);
	printk("usb_interrupt_level2:0x%x\n",usb_interrupt_level2);
				
		if (usb_interrupt_level2 & BIT0)
		       {
#ifndef CONFIG_USB_OTG_SL2312_1
		       	mUsbIntBusRstClr();
 ...
 //----- Group Byte 1 ---------
	if (dev->usb_interrupt_level1 & BIT1)			
	{
	   ...
	   printk("(IntSCR1:0x%x)(Mask1:0x%x)(usb_interrupt_level2=0x%x)\n"
...
static void FTC_gpio_irq (void)
{
	unsigned int    reg_val; 
	unsigned int addr,addr1;
        unsigned int value,value1;
        
	addr = GPIO_BASE_ADDR + 0x08; //VBUS on/off pin.
	value = readl(addr) & 0xfffbffff;
	writel(value,addr);
	addr1 = GPIO_BASE_ADDR + 0x04;
	value1 = readl(addr1);
        value1 &= 0x00040000;
	
	if (value1) //GPIO #18 1=VBUS off
	{
	reg_val = readl(SL2312_GLOBAL_BASE_ADDR + 0x30);
	reg_val &= 0xff7fffff; //USB1 gadget VBUS off !!!
	writel(reg_val,SL2312_GLOBAL_BASE_ADDR + 0x30);
	printk("******Enable USB1 Vbus off!!!!\n");	
        }
        else{	 //GPIO #18 0=VBUS on
	reg_val = readl(SL2312_GLOBAL_BASE_ADDR + 0x30);
	reg_val = reg_val | 0x00800000; //USB1 gadget VBUS on !!!
	writel(reg_val,SL2312_GLOBAL_BASE_ADDR + 0x30);
	printk("******Enable USB1 Vbus on !!!!\n");
         }
}
...
static void FTC_usb_remove(void)//FOTG200.ok
{
	...
	if (the_controller_1->driver) {
		/* should have been done already by driver model core */
		WARN(dev,"remove driver '%s' is still registered\n",
				the_controller_1->driver->driver.name);
		usb_gadget_unregister_driver(the_controller_1->driver);
	}

	udc_reset(the_controller_1);

//#ifdef OTG200_Peripheral_Only  
#ifndef CONFIG_USB_OTG_SL2312_1
     if (the_controller_1->got_irq)  //Andrew update
         free_irq(IRQ_FOTG200, the_controller_1);//Andrew update
#endif



	// free EP0 req, buffer
	//Andrew update;;FTC_free_buffer (&the_controller_1->ep[0].ep, 
	//Andrew update;;                 the_controller_1->EP0req->buf, 
	//Andrew update;;				 the_controller_1->EP0req->dma, the_controller_1->EP0req->length);
	//Andrew update;;FTC_free_request (&the_controller_1->ep[0].ep, the_controller_1->EP0req);

   kfree(the_controller_1); //Andrew update
	the_controller_1 = 0;
 ...
 */
static int FTC_usb_probe_1(void)//FOTG200.ok
{
 int retval=0;
    u32 wFound=0;
    int		rc;
	...
	DBG_FUNCC("+FTC_usb_probe_1()\n");
#ifndef CONFIG_USB_OTG_SL2312_1    
unsigned int    reg_val;   //stone set global reg 0x30 to mini-A Host
    reg_val = readl(SL2312_GLOBAL_BASE_ADDR + 0x30);
//    reg_val &= 0xbf7fffff; //USB1 
    reg_val = reg_val | 0x40000000; //USB1 gadget support !!!
    reg_val &= 0xff7fffff; // USB1 VBUS off, use GPIO #18 detect(Low-active level trig)!!!
    printk("******reg_val = %x \n",reg_val);
    writel(reg_val,SL2312_GLOBAL_BASE_ADDR + 0x30);
#endif
...
//    wFOTGPeri_Port(0xC4)|=0x06; //Disable Host & Control interrupt
  ...
  //<2>.Init "the_controller_1" structure
	...
	the_controller_1 = kmalloc (sizeof *the_controller_1, SLAB_KERNEL);
	if (the_controller_1 == NULL){
		pr_debug("enomem USB device\n");
		retval = -ENOMEM;
		goto done;
	}

	memset(the_controller_1, 0, sizeof *the_controller_1);
	spin_lock_init(&the_controller_1->lock);
	the_controller_1->gadget.ops = &FTC_ops;

	/* the "gadget" abstracts/virtualizes the controller */
//	the_controller_1->gadget.dev.bus_id = "gadget";
	the_controller_1->gadget.name = driver_name;
#ifdef CONFIG_USB_OTG_SL2312_1  
	the_controller_1->gadget.udc_isr = udc_irq_for_OTG;
#else
        the_controller_1->gadget.udc_isr = FTC_irq;
#endif
	the_controller_1->enabled = 1;


	the_controller_1->EPUseDMA = DMA_CHANEL_FREE;   //reset

 //<3>. udc Reset/udc reinit
	/* init to known state, then setup irqs */
	udc_reset(the_controller_1);
	udc_reinit(the_controller_1);
...
//    cpe_int_set_irq(IRQ_FOTG200, LEVEL, H_ACTIVE);
	      ...
	      if (register_chrdev(DEV_MAJOR, "FOTG2XX", &dev_fops)) 
	{
//		DBG(dev, "request interrupt failed\n");
	...
	the_controller_1->gadget.dev.parent = NULL;
      	strcpy (the_controller_1->gadget.dev.bus_id, "FTC_FOTG200_udc_1");
	the_controller_1->gadget.dev.dma_mask = (void *)0xffffffff;
	the_controller_1->gadget.dev.coherent_dma_mask = 0xffffffff;
	if (!request_mem_region(0x69000000,0x1000, "FTC_FOTG200_udc_1")) {
		printk("request_mem_region failed");
		return -EBUSY;
	        }
	rc=device_register (&(the_controller_1->gadget.dev));
	if (rc < 0)
	printk("*****Can't register device !!!!!\n");
#endif

	the_controller_1->got_irq = 0; //stone

//#ifndef OTG200_Peripheral_Only 
#ifdef CONFIG_USB_OTG_SL2312_1 
    xceiv = FOTG2XX_get_otg_transceiver_1();//For OTG
    the_controller_1->transceiver = xceiv;//For OTG
    the_controller_1->gadget.is_otg = 1;
#endif    
    
        printk("FTC_usb_probe_1 end\n");
	/* done */
	return 0;
...
static int __init init (void)//FOTG200.ok
{
	...
	printk("FOTG200_BASE_ADDRESS = 0x%x\n", FOTG200_BASE_ADDRESS);
	return FTC_usb_probe_1();
}
module_init (init);
...
static int usb_gadget_register_driver_1(struct usb_gadget_driver *driver)
{
	struct FTC_udc	*dev = the_controller_1;
	int	   retval,status,reg_val;
    //u8 i;
 
	printk("+usb_gadget_register_driver_1()\n");
	...
	wFOTGPeri_Port(0xC4)|=0x06;//Fource FOTG200-Interrupt to High-Active
...
retval = request_gpio_irq(18,FTC_gpio_irq,0,1,1); //USB 0/1 1.VBUS on/off 2. ID GPIO detect.
if (retval)
		printk("GPIO interrupt init error.\n");

wFOTGPeri_Port(0xC4)|=0x08;//Fource FOTG200-Interrupt to High-Active

if (request_irq(IRQ_FOTG200, FTC_irq, SA_INTERRUPT /*|SA_SAMPLE_RANDOM*/,
		            driver_name, the_controller_1) != 0) 
	{
...
EXPORT_SYMBOL(usb_gadget_register_driver_1);
