...
//Sunny 2007.02.07 begin
// W83627EHG register
#define UART_RBR        0       /* In:  Receive Buffer Register(DLAB=0) */
#define UART_TBR        0       /* Out: Transmit Buffer Register(DLAB=0) */
#define UART_BLL        0       /* Baudrate Divisor Latch Low (DLAB=1) */
#define UART_BHL        1       /* Baudrate Divisor Latch High (DLAB=1) */
#define UART_ICR        1       /* Interrupt Control Register */
#define UART_UFR        2       /* Out: UART FIFO Control Register */
#define UART_UCR        3       /* Out: Uart Control Register */
#define UART_HCR        4       /* Handshake Control Register */
#define UART_USR        5       /* UART Status Register */

//These are the definitions for the UART Control Register of W83627EHG
#define UART_UCR_BDLAB  0x80    /* Baudrate Divisor Latch Access Bit */
#define UART_UCR_STOP   0x04    /* Stop bits: 0=1 stop bit, 1= 2 stop bits */
#define UART_UCR_WLEN8  0x03    /* WordLength: 8 bits */

//These are the definitions for the UART Status Register of W83627EHG
#define UART_USR_RDR    0x01    /* RBR Data Ready */
#define UART_USR_TBRE   0x20    /* Transmit Buffer Register Empty */

 static void init_W83627EHG_uartA(void)
{
        ...
        WRITE_UINT8(LPC_KEY_ADDR, 0x30);
        WRITE_UINT8(LPC_DATA_ADDR, 0x01);
        WRITE_UINT8(LPC_KEY_ADDR, 0x60);
        WRITE_UINT8(LPC_DATA_ADDR, 0x03);
        WRITE_UINT8(LPC_KEY_ADDR, 0x61);
        WRITE_UINT8(LPC_DATA_ADDR, 0xf8);
        WRITE_UINT8(LPC_KEY_ADDR, 0xaa);
}//Sunny 2007.02.07 end
...
//Sunny 2007.02.07
	 // init UART A
        init_W83627EHG_uartA();

        READ_UINT8(base + UART_UCR, lcr);
        lcr &= ~UART_UCR_BDLAB;
        WRITE_UINT8(base + UART_UCR, UART_UCR_BDLAB);// enter BHL,BLL
        WRITE_UINT8(base + UART_BHL, 6 >> 8);
        WRITE_UINT8(base + UART_BLL, 6);
        lcr &= 0xc0;
        lcr |= UART_UCR_STOP;//stop bit is 2
        lcr |= UART_UCR_WLEN8;//word length is 8
        HAL_WRITE_UINT8(base + UART_UCR, lcr);
        HAL_WRITE_UINT8(base + UART_HCR, 0x08);//enable IRQ
        HAL_WRITE_UINT8(base + UART_UFR, 0x01);//enable FIFO
        HAL_WRITE_UINT8(base + UART_ICR, 0x05); //enable RBR,USR interrupt
