...
//			}
	...
	else if ((ipsec_tunnel_ptr->mode == 0) && (ipsec_tunnel_ptr->protocol == IPPROTO_AH))
	{
		...
		top_ip_hdr = (struct iphdr*)(skb->data + ENET_HEADER_SIZE);
		ip_hdr = (struct iphdr*)(skb->data + ENET_HEADER_SIZE + IP_HEADER_SIZE + 
					sizeof(struct ip_auth_hdr) + ipsec_tunnel_ptr->icv_trunc_len);
		memcpy(crypto_ops->iv,skb->data+IP_HEADER_SIZE+sizeof(struct ip_auth_hdr)+ipsec_tunnel_ptr->icv_trunc_len,ENET_HEADER_SIZE);
		...
		top_ip_hdr->ihl = 5;
		top_ip_hdr->tos = 0;
		top_ip_hdr->tot_len = htons(skb->len - ENET_HEADER_SIZE);
		...
		ah_hdr = (struct ip_auth_hdr *)(skb->data + ENET_HEADER_SIZE + sizeof(struct iphdr));
		...
		memset(skb->data + ENET_HEADER_SIZE + sizeof(struct iphdr) + sizeof(struct ip_auth_hdr),0x0,ipsec_tunnel_ptr->icv_trunc_len);
		...
		crypto_ops->auth_algorithm = ipsec_tunnel_ptr->auth_alg;
		crypto_ops->auth_result_mode = AUTH_APPEND;
		crypto_ops->auth_header_len = ENET_HEADER_SIZE;
		crypto_ops->auth_algorithm_len = skb->len - ENET_HEADER_SIZE;
		crypto_ops->auth_check_len = (unsigned int)(ipsec_tunnel_ptr->icv_trunc_len / 4);
//		crypto_ops->auth_checkval = (unsigned char*)(skb->data+skb->len-ipsec_tunnel_ptr->icv_trunc_len);
		crypto_ops->icv_trunc_len = ipsec_tunnel_ptr->icv_trunc_len;
		crypto_ops->icv_full_len = ipsec_tunnel_ptr->icv_full_len;
	...
	else if ((ipsec_tunnel_ptr->mode == 1) && (ipsec_tunnel_ptr->protocol == IPPROTO_AH))
	{
		...
		memcpy(temp_ip_hdr_copy,skb->data+ENET_HEADER_SIZE,12);
		top_ip_hdr->tos = 0;
		top_ip_hdr->frag_off = 0;
		top_ip_hdr->ttl = 0;
		top_ip_hdr->check = 0;
		...
		memcpy(temp_auth_check_val,skb->data+ENET_HEADER_SIZE+sizeof(struct iphdr)+sizeof(struct ip_auth_hdr)
...
void ipsec_finish_callback(struct IPSEC_PACKET_S *ipsec_ptr)
{
	...
	if (ipsec_ptr->op_mode == ENC_AUTH)
	{
		...
		skb->nh.iph = (struct iphdr*)(&skb->data[14]);
		skb->nh.iph->tot_len = htons(skb->len - ENET_HEADER_SIZE);
		skb->nh.iph->ttl = 128;
	...
	if (ipsec_ptr->op_mode == AUTH)
	{
		if (ipsec_ptr->auth_result_mode == AUTH_APPEND)
		{
			...
			ip_hdr2 = (struct iphdr*)(skb->data + ENET_HEADER_SIZE+sizeof(struct iphdr)+
			...
			memcpy(skb->data+ENET_HEADER_SIZE+sizeof(struct iphdr)+sizeof(struct ip_auth_hdr),
...
//		printk("%s::sending new packet\n",__func__);
		...
		((__u8)skb->cb[28]) = 1;
		if ((skb->len != (ntohs(skb->nh.iph->tot_len) +14)) || (skb->len < 42))
		{
...
//			ip_hdr->tot_len = htons(old_skb->len - ENET_HEADER_SIZE);
			ip_hdr->ttl++;
			ip_send_check(ip_hdr);
		}
		...
		{
			memcpy(old_skb->data+ENET_HEADER_SIZE,ipsec_ptr->iv,12);
			memcpy(old_skb->data+ENET_HEADER_SIZE+sizeof(struct iphdr)+sizeof(struct ip_auth_hdr),
