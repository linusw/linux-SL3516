...
//debug_Aaron on 05/30/2008 for IxLoad
       // dev->weight = 64;
        dev->weight = 128;
#endif
		...
		else
		{
			...
			if (total_len <= 60){
				...
				if(likely(eth->h_proto == __constant_htons(ETH_P_IP)&& \
					...
					iph_tot_len = ((ip_hdr->tot_len & 0xFF)<< 8) | ((ip_hdr->tot_len & 0xFF00)>>8);
					if((iph_tot_len+ETH_HLEN)<total_len)
						total_len = iph_tot_len+ETH_HLEN;
				...
				*/
				...
				// Jason, tell GMAC not to modify ip_hdr->tot_len 
				if(frag_id==0)
...
#if defined(CONFIG_SL351X_FAST_VPN)
				if(likely(sl_fast_ipsec(skb))) {
...
#if defined(CONFIG_SL351x_FAST_NET) || defined(CONFIG_SL351X_FAST_VPN)
skb_handled2:
#endif
				dev->last_rx = jiffies;
				isPtr->rx_bytes += pkt_size;
	...
	int max_pkts_num = rx_pkts_num;

#ifdef CONFIG_SL351X_IPSEC
	// Handle classfication queue for ipsec VPN
	int classq_pkts_num=0;
