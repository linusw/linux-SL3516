...
#define SATA_HOT_PLUG_ENABLE		0x04
...
static void lepus_sata_enable_hotplug(struct ata_probe_ent *probe_ent);
static void lepus_sata_disable_hotplug(struct ata_host_set *host_set);
static void lepus_sata_check_hotplug(struct ata_host_set *host_set);
...
void lepus_phy_reset (struct ata_port *ap)
{
	...
	do{
		...
		if((status_p1==0)||((status_p1==0)))
			break;
	}while (time_before(jiffies, timeout));
	
	if((status_p0&0x05)||(status_p1&0x05))		// device attach and link estabilished
		ata_port_probe(ap);
	else
		ata_port_disable(ap);
...
static int lepus_sata_suspend(struct device *dev, u32 state, u32 level)
{
...
static int lepus_sata_resume(struct device *dev, u32 state)
{
	printk("%s: not implemented yet\n", __FUNCTION__);
...
void lepus_sata_enable_hotplug(struct ata_probe_ent *probe_ent)
{
/*	
	unsigned int reg_v;
	
	reg_v = inl(probe_ent->port[0].scr_addr + SATA_CONTROL_REG0);
	reg_v |= SATA_HOT_PLUG_ENABLE;
	outl(reg_v,probe_ent->port[0].scr_addr + SATA_CONTROL_REG0);
	
	outb(SATA_ENABLE_CMD0, probe_ent->port[0].device_addr );
	outb(SATA_ENABLE_CMD1, probe_ent->port[0].device_addr );
*/
}

 void lepus_sata_disable_hotplug(struct ata_host_set *host_set)
{
/*
	unsigned int reg_v;
	
	reg_v = inl(host_set->ports[0]->ioaddr.scr_addr + SATA_CONTROL_REG0);
	reg_v &= ~SATA_HOT_PLUG_ENABLE;
	outl(reg_v, host_set->ports[0]->ioaddr.scr_addr + SATA_CONTROL_REG0);

	outb(SATA_DISABLE_CMD0, host_set->ports[0]->ioaddr.device_addr );
	outb(SATA_DISABLE_CMD1, host_set->ports[0]->ioaddr.device_addr );
*/
}

void lepus_sata_check_hotplug(struct ata_host_set *host_set)
{
	u8 intr_status;

	intr_status = inb(host_set->ports[0]->ioaddr.scr_addr + lepus_sata_INT_STATUS);

	// Clear interrupt status.
	outb(0xff, host_set->ports[0]->ioaddr.scr_addr + lepus_sata_INT_STATUS);

	if (intr_status & lepus_sata_INT_STATUS_HOTPLUG) {
		if (intr_status & lepus_sata_INT_STATUS_PDEV_ADDED)
			printk(KERN_WARNING "lepus_sata_sata: "
				"Primary device added\n");

		if (intr_status & lepus_sata_INT_STATUS_PDEV_REMOVED)
			printk(KERN_WARNING "lepus_sata_sata: "
				"Primary device removed\n");

		if (intr_status & lepus_sata_INT_STATUS_SDEV_ADDED)
			printk(KERN_WARNING "lepus_sata_sata: "
				"Secondary device added\n");

		if (intr_status & lepus_sata_INT_STATUS_SDEV_REMOVED)
			printk(KERN_WARNING "lepus_sata_sata: "
