...
// for some weird reasons, sometimes the performace for IPSEC-VPN is better with 
// this feature enable; however, it has some mechanism that will check if the 
// current crypto packet sending is batch or single.  Even if user enables this 
// feature, but not using it. It will still follow single crypto packet processing.
...
//static spinlock_t		ipsec_rx_lock;
...
#if defined(CONFIG_SL2312_IPSEC_NAPI) || defined(CONFIG_SL2312_IPSEC_INTERRUPT)
...
static void ipsec_rx_packet(unsigned int mode);
...
#ifdef CONFIG_SL2312_IPSEC_NAPI
static irqreturn_t ipsec_interrupt(int irq, void *dev_id, struct pt_regs *regs);
static void ipsec_napi_func(unsigned long data);
#endif

#ifdef CONFIG_CRYPTO_BATCH
static void process_ipsec_recursive(struct IPSEC_PACKET_S *crypto_queue,int count, int current_count, int loc);
...
static void ipsec_hw_cipher_vpn(volatile unsigned char *ctrl_pkt,int ctrl_len, volatile unsigned char *data_pkt, 
					 int data_len, unsigned int tqflag, unsigned char *out_pkt,int *out_len,int count);
...
//		printk("%s::tasklet has been scheduled! watch out for conflict\n",__func__);
//	}
...
#if !defined(CONFIG_SL2312_IPSEC_INTERRUPT) && !defined(CONFIG_SL2312_IPSEC_NAPI)
...
//	unsigned int			own; 
    ...
    for (;;)
    {
//		consistent_sync(rx_desc,sizeof(IPSEC_DESCRIPTOR_T),DMA_FROM_DEVICE);
		// debug message
//		if (rx_desc == NULL)
//		{
...
//		if (__pa(op_info->out_packet) != rx_desc->buf_adr) {
//			printk("%s::op_info->out_packet(%x) vs rx_desc->buf_adr(%x)\n",
//				__func__,__pa(op_info->out_packet),rx_desc->buf_adr);
//		}
//		if (op_info->out_packet != op_info->in_packet2) {
//			printk("%s::op_info->out_packet(%x) vs op_info->in_packet2(%x)\n",
//				__func__,op_info->out_packet,op_info->in_packet2);
//		}
		...
		if ((op_info = ipsec_get_queue(ipsec_queue))!=NULL)
		{
			...
			consistent_sync(op_info->out_packet,pkt_len,DMA_FROM_DEVICE);
			mb();
//			consistent_sync(op_info->out_packet,pkt_len,DMA_FROM_DEVICE);
...
//			spin_unlock_irqrestore(&ipsec_rx_lock,flags_a);
			...
			if ((polling_flag == 1 ) && ((int)process_id == polling_process_id)) {
				...
				spin_unlock_irqrestore(&ipsec_polling_lock,flags);
//				printk("%s::done polling process_id %d\n",__func__,process_id);
			}
//			else if (polling_flag) {
//				printk("process_id %d vs polling_process_id %d\n",process_id, polling_process_id);
				// need to run poll here?
//			}
//			else if (process_id == polling_process_id)
//				printk("%s::. hm polling_flag is not on o_O\n",__func__);
#endif
			count++;
		}
		else
...
//			printk("%s::length of the input = %d\n",__func__,op_info->pkt_len);
//			printk("%s::length of the result = %d\n",__func__,op_info->out_pkt_len);
//			printk("%s::return status = %d\n",__func__,op_info->status);
...
//		return;
//	}
//	printk("%s::got here, going to handle with NAPI mode\n",__func__);
...
static irqreturn_t ipsec_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	{
//		printk("status = %16x\n",status.bits32);
//		printk("ipsec_interrupt : going to ipsec_complete_tx_packet\n");
		ipsec_complete_tx_packet();
	}
	if (status.bits.rs_eofi==1)
	{
//		printk("ipsec_interrupt : going to ipsec_rx_packet\n");
		ipsec_rx_packet(0);
//		mark_bh(RISCOM8_BH);
...
//	{
//		printk("status = %16x\n",status.bits32);
//		printk("%s:: going to ipsec_complete_tx_packet\n",__func__);
//		ipsec_complete_tx_packet();
//	}
...
//		{
//			if (status.bits.ts_eofi==1)
//				ipsec_info->status.bits.ts_eofi = 1;
//		}
	}
#endif
	enable_irq(IRQ_IPSEC);
...
//	printk("%s::polling_flag(%d), polling_prcoess_id(%d), last_rx_pid(%d)\n",
//		__func__,polling_flag, polling_process_id, last_rx_pid);
...
int process_ipsec_batch(struct IPSEC_PACKET_S *crypto_queue, int count, int start_loc)
{
	int available_space = desc_free_space();
	int result = 0;
	unsigned long flags;
...
//	printk("%s::going to process %d packets\n",__func__,count);
	tx_desc_count = 0;
//	ipsec_complete_tx_packet();

	spin_lock_irqsave(&ipsec_tx_lock,flags);
	process_ipsec_recursive(crypto_queue, count, 0, start_loc);
	spin_unlock_irqrestore(&ipsec_tx_lock,flags);
//	printk("\n\n");
//	ipsec_complete_tx_packet();
...
//		printk("%s::count = %d, current_count = %d\n",__func__,count,current_count);
		cur_loc = current_count + loc;
		if (cur_loc >= CRYPTO_QUEUE_SIZE)
			cur_loc -= CRYPTO_QUEUE_SIZE;
		struct IPSEC_PACKET_S * packet_ptr = (struct IPSEC_PACKET_S*)&(crypto_queue[cur_loc]);
//		printk("%s:: current_count = %d, pid = %d, cur_loc = %d\n",__func__,current_count,pid,cur_loc);
...
//		consistent_sync(rx_desc,sizeof(IPSEC_DESCRIPTOR_T),DMA_BIDIRECTIONAL);
		...
		if (current_count == 0) {
			...
			//printk("%s %x rx_desc = %x\n", __func__, rxdma_desc, rx_desc);
					...
					switch (op->auth_algorithm)
					{
						case MD5:
							...
							ipsec_hw_cipher_vpn((unsigned char *)cbc_auth_ptr,control_packet_len,
									op->in_packet2,op->pkt_len,tdflag,
									ipsec_result.hw_cipher,&ipsec_result.hw_pkt_len,count);
							break;
...
//static void ipsec_hw_cipher_vpn(unsigned char *ctrl_pkt,int ctrl_len, struct scatterlist *data_pkt, 
//					 int data_len, unsigned int tqflag, unsigned char *out_pkt,int *out_len,int count)
static void ipsec_hw_cipher_vpn(volatile unsigned char *ctrl_pkt,int ctrl_len, volatile unsigned char *data_pkt, 
					 int data_len, unsigned int tqflag, unsigned char *out_pkt,int *out_len,int count)
...
//	IPSEC_DMA_STATUS_T	ipsec_dma_status;
	...
	for (i=0; i<(count+count); i++) {
		tx_desc = (IPSEC_DESCRIPTOR_T *)((tx_desc->next_desc.next_descriptor & 0xfffffff0)+tx_desc_virtual_base);
		if (tx_desc->frame_ctrl.bits.own != CPU) {
			printk("%s::Not enough TX Descriptors a\n",__func__);
			ipsec_read_reg(0x0000);
		}
	}
...
//	printk("%s::count == %d\n",__func__,count);
	...
	tx_desc = (IPSEC_DESCRIPTOR_T *)((tx_desc->next_desc.next_descriptor & 0xfffffff0)+tx_desc_virtual_base);
	if (tx_desc->frame_ctrl.bits.own != CPU) {
		printk("%s::Not enough TX Descriptors b\n",__func__);
		ipsec_read_reg(0x0000);
	}
	...
	// debug message
//	printk("%s::Process ID = %d\n",__func__,((IPSEC_CBC_AUTH_T *)ctrl_pkt)->control.bits.process_id);
...
//			printk("%s::status.bits.ts_eodi = %d!\n",__func__,ipsec_dma_status.bits.ts_eodi);
				break;
		}
	}
...
//		printk("%s::going to fill in desc@%x\n",__func__,desc);
 ...
 * Return: Number of rxq_available_space - txq_used_desc_pair
...
static int desc_free_space(void)
{
	...
	unsigned int rx_space, tx_used_desc;
	...
	if ((unsigned int)tx_desc_wptr >= (unsigned int)tx_desc_rptr)
		tx_used_desc = ((unsigned int)tx_desc_wptr - (unsigned int)tx_desc_rptr) / sizeof(IPSEC_DESCRIPTOR_T);
	else
		tx_used_desc = IPSEC_TX_DESC_NUM - (((unsigned int)tx_desc_rptr - (unsigned int)tx_desc_wptr) / sizeof(IPSEC_DESCRIPTOR_T));
	tx_used_desc = tx_used_desc>>1;

	return (int)(rx_space - tx_used_desc);
}
#endif
