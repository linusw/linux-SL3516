...
int sl_fast_bridging(struct sk_buff *skb, int in_device_gmac_index) {
...
#if 0
	// TODO: this case shouldn't ever happen. Remove after testing.
	// make sure skb is a least ETH_HLEN  long
	if(unlikely(skb->len < ETH_HLEN)) {
		printk("ERROR %s %s :: skb->len(%d) < ETH_HLEN\n",
			skb->dev->name, __func__, skb->len);
		return 0;	
	}
#endif
	...
	if( unlikely(br_fdb = br_fdb_get(br_port->br, &(skb->data[0]) )) == NULL )
		goto no_hash_1;

	if( unlikely(br_fdb_sa = br_fdb_get(br_port->br, &(skb->data[6]) )) == NULL )
		goto no_hash_2;
...
**/
	...
	if( likely(eth->h_proto == __constant_htons(ETH_P_IP)) && 
		...
		skb->nh.iph = iph;
		// make sure skb lenth is at least as long as indicated in iph
		if( unlikely( skb->len < (ntohs(iph->tot_len)+ETH_HLEN)) ) {
		
			printk("ERROR %s %s:: skb->len(%d) < (iph->tot_len + ETH_HLEN)(%d)\n", 
				skb->dev->name, __func__, skb->len, ntohs(iph->tot_len)+ETH_HLEN);
			return 0;
		}
		...
		// Because some of the NAT related functions works in IP layer, we move skb->data to IP layer
		skb->data+=ETH_HLEN;
		skb->len-=ETH_HLEN;
...
#else
			struct ip_conntrack_tuple_hash *th;
			th = __ip_conntrack_find(&tuple, NULL);
			if (!th) {
//				printk("%s::h is null!\n", __func__);
				goto skip_fast_nat;
			}
...
//			printk("6\n");
			...
			// Move back to MAC layer
			skb->data-=ETH_HLEN;
			skb->len+=ETH_HLEN;

			// Since we are at MAC layer, we do the following				
			memcpy(skb->data - RX_INSERT_BYTES, hh->hh_data, hh_alen);
...
// If the skb origin from gmac and destinate to gmac then we need to skip handling this packet.
if(unlikely(in_device_gmac_index!=-1) && toe->gmac[1-in_device_gmac_index].dev == skb->dev) {
	return 0;
}
#endif
...
//			printk("Packet NATed\n");
		...
		skip_fast_nat:;
		//printk("Fast NAT skiped\n");

		// Move skb->data back to MAC layer
		skb->data-=ETH_HLEN;
		skb->len+=ETH_HLEN;
