...
unsigned short wan_jumbo_frame_size = 1500;
unsigned short lan_jumbo_frame_size = 1500;
unsigned short max_jumbo_frame_size = 1500;
...
int NICSTATUS=0;	//NICSTATUS For QC Mode
/*************************************************************
 *         Global Variable
 *************************************************************/
...
#ifdef  L2_jumbo_frame
static int gmac_change_mtu_jumboframe(struct net_device *dev, int new_mtu);
extern int get_jumbo_frame_size(int port);
#else
...
/* 2007.08.09, Neagus */
#ifdef  L2_jumbo_frame
        wan_jumbo_frame_size = (unsigned short)get_jumbo_frame_size(0);
        lan_jumbo_frame_size = (unsigned short)get_jumbo_frame_size(1);

	if(wan_jumbo_frame_size >= lan_jumbo_frame_size)
		max_jumbo_frame_size = wan_jumbo_frame_size;
	else
		max_jumbo_frame_size = lan_jumbo_frame_size;
#endif
...
//	init_waitqueue_entry(&wait, current);
	...
	for(i = CONFIG_MAC_NUM-1; i>=0; i--)		// 20070317 swap netdevice sequence
...
/* 2007.08.09, Neagus */
#ifdef  L2_jumbo_frame
	unsigned short jumbo_frame_size;
	if(tp->port_id == 1){
		/* WAN - GMAC_PORT1 */
		jumbo_frame_size = wan_jumbo_frame_size;
	}else if(tp->port_id == 0){
		/* LAN - GMAC_PORT0 */
		jumbo_frame_size = lan_jumbo_frame_size;
	}
		if (jumbo_frame_size > 1500)
                        dev->mtu = 2018; //2018
                else
                        dev->mtu = 1500;

		dev->change_mtu = gmac_change_mtu_jumboframe;
#else
...
/*
        if(get_model())
		max_phy=2;	//N299, control WAN/LAN LED
	else
		max_phy=1;	//N199, don't control LAN(10/100 Phy) LED
		
        for(phy_addr=1; phy_addr<=max_phy; phy_addr++){
*/
        for(phy_addr=1; phy_addr<=2; phy_addr++){
                if(sl2312_qc_stat()){
                        ...
                        mii_write(phy_addr, 0x16, 0x3); //page address
                        mii_write(phy_addr, 0x10, 0x477);  //blink->activity, off->no activity
                }else{
                        mii_write(phy_addr, 0x16, 0x3); //page address
                        mii_write(phy_addr, 0x11, 0x0000); //low active
                        mii_write(phy_addr, 0x16, 0x3); //page address
	...
	if (!gmac_initialized)
	{
        ...
        if(get_model()){
        	toe->gmac[1].phy_addr = 0x2;//For H/W of N299, by Neagus
	}else{
        	toe->gmac[1].phy_addr = 0;//Remark N199 10/100 phy, Neagus
	}
...
static void toe_init_gmac(struct net_device *dev)
{
   ...
   /* get mac address from FLASH */
	...
	//mii_write(tp->phy_addr,0x10,0x400);	//Neagus, active led
...
//              if (skb->nh.iph->protocol == 0x006 && (skb->nh.iph->protocol == __constant_htons(ETH_P_IP)))
		/* Neagus 2007.08.21 */
                if ((skb->nh.iph->protocol == IPPROTO_TCP) && skb->h.th)
		//if ((skb->nh.iph->protocol == IPPROTO_TCP) && (skb->protocol != htons(ETH_P_ARP)) && (skb->protocol != htons(ETH_P_PPP_SES)))
                {
...
//                                                printk("-----> Change MSS to 8K \n" );
						...
						if(tp->port_id == 1){
							/* WAN - GMAC_PORT1 */
							ptr[2] = htons(wan_jumbo_frame_size - 40);
						}else if(tp->port_id == 0){
							/* LAN - GMAC_PORT0 */
							ptr[2] = htons(lan_jumbo_frame_size - 40);
						}
                                        }
                                }
                }
...
#else
                //word3 = 1514 | EOFIE_BIT;	//BSP 252
		word3 = (dev->mtu+14) | EOFIE_BIT;	//BSP 262
#endif

#ifdef DO_HW_CHKSUM	
#ifdef  L2_jumbo_frame
		//Neagus 2007.09.04
                if (total_len <= (dev->mtu+14) && skb->nh.iph && (skb->nh.iph->frag_off & __constant_htons(0x3fff)))
#else
...
static void gmac_get_mac_address(void)
{
    ...
    {
	/* Neagus 20070529, Change de MAC address for N199 */
        if(get_model()){        //N299
                memcpy((void *)&eth_mac[1][0],vlan[0].mac,6);
                // VLAN_conf[0].vid = vlan[0].vlanid;
                // VLAN_conf[0].portmap = vlan[0].vlanmap;
                memcpy((void *)&eth_mac[0][0],vlan[1].mac,6);
...
static int gmac_open (struct net_device *dev)
{
	...
	/* link down, turn off the watch dog */
...
/*
       printk("**************************\n");
        ...
        mii_write(tp->phy_addr, 0x16, 0x3);
        test_val = mii_read(tp->phy_addr, 0x12);
        printk("led time control REG18=0x%04x\n", test_val);

        mii_write(tp->phy_addr, 0x16, 0x3);
        test_val = mii_read(tp->phy_addr, 0x11);
        printk("led polarity control REG17=0x%04x\n", test_val);

        mii_write(tp->phy_addr, 0x16, 0x3);
        test_val = mii_read(tp->phy_addr, 0x10);
        printk("led function control REG16=0x%04x\n", test_val);

        //set led timer control
        mii_write(tp->phy_addr, 0x16, 0x3); //page address
        mii_write(tp->phy_addr, 0x12, 0x0000); //default
        //set led polarity
        mii_write(tp->phy_addr, 0x16, 0x3); //page address
        mii_write(tp->phy_addr, 0x11, 0x0000); //low active
        //set led function
        mii_write(tp->phy_addr, 0x16, 0x3); //page address
        mii_write(tp->phy_addr, 0x10, 0x888);  //blink->activity, off->no activity
...
void gmac_get_phy_status(struct net_device *dev)
{
	...
	if ((reg_val & 0x0024) == 0x0024) /* link is established and auto_negotiate process completed */
	{
		...
		if (tp->pre_phy_status == LINK_DOWN)
		{
			...
			//Sunny 20070320
                if(sl2312_qc_stat()){
			write_kernellog("%d %s network was %s", INFO_LOG, 
	...
	else
	{
		...
		if (tp->pre_phy_status == LINK_UP)
		{
			...
			//Sunny 20070320
                if(sl2312_qc_stat()){
			write_kernellog("%d %s network was %s", WARN_LOG, 
				(!strcmp(dev->name, "eth0")) ? 
				"WAN" : "LAN", "unplugged");
		}else{
			//QC mode
