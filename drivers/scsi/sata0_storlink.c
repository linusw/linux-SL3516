...
//+ Andge 20080417
#define SATA_STATE_INACTIVE     0
#define SATA_STATE_ACTIVE       1
#define SATA_STATE_STANDBY      2

pid_t   PM0_Pid=0;
unsigned long   IdleTime=0;     /* minute */
struct device    *SataPortDev0;
int     SataRWState0 = SATA_STATE_ACTIVE;
static struct timer_list pm_timer;        /* state timer */
static int  MinuteCount;


static void pm_timer_run( unsigned long parameters )
{
    unsigned long   ioaddr = parameters;


    //printk("[pm_timer_run 0:  SataRWState0 = %d ]\n", SataRWState0);
    switch( SataRWState0 )
    {
        case SATA_STATE_INACTIVE:
            MinuteCount++;
            if( MinuteCount >= IdleTime )
            {
                MinuteCount = 0;
                /* Set SATA HDD to STANDBY immediately */
                printk("\n---------- SATA-0 STANDBY Pid %d -----------\n", PM0_Pid);
                if( PM0_Pid )
                    kill_proc( PM0_Pid, SIGUSR2, 1 );
                SataRWState0 = SATA_STATE_STANDBY;
            }
            break;
        case SATA_STATE_ACTIVE:
            SataRWState0 = SATA_STATE_INACTIVE;
            MinuteCount = 0;
            break;
        case SATA_STATE_STANDBY:
        default:
            break;
    }

	pm_timer.function = (void *)pm_timer_run;
	pm_timer.data = ioaddr;
	pm_timer.expires = jiffies + HZ*60;
    add_timer( &pm_timer );
}



EXPORT_SYMBOL(PM0_Pid);
EXPORT_SYMBOL(IdleTime);
EXPORT_SYMBOL(SataPortDev0);
EXPORT_SYMBOL(SataRWState0);
//-
...
void lepus_sata_setup_port0(struct ata_ioports *port, unsigned long base)
{
...
//	writel(0x03,IO_ADDRESS(SL2312_SATA_BASE)+0x18);					// Enable SATA salve mode	
	...
	lepus_sata_setup_port0(&probe_ent->port[0], probe_ent->mmio_base );
...
//+ Andge   20080417
    ...
    init_timer( &pm_timer );
	pm_timer.function = (void *)pm_timer_run;
	pm_timer.data = probe_ent->port[0].command_addr;
	pm_timer.expires = jiffies + HZ*60;
    add_timer( &pm_timer );
//-
	return 0;
...
static int lepus_sata0_suspend(struct device *dev, u32 state, u32 level)
{
...
static int lepus_sata0_resume(struct device *dev, u32 state)
{
	printk("%s: not implemented yet\n", __FUNCTION__);
